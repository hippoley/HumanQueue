...
Human Queue separates **implemented code**, **verified behavior**, **real-host validation**, and **product hypotheses**.

A green checkbox here means only what the evidence column says it means.

| Claim | Status | Evidence | Current limit |
| --- | --- | --- | --- |
| Canonical Human Queue request lifecycle works locally | **verified in CI** | queue / resolve / quorum / supersession tests | Does not prove any external runtime consumes a decision |
...
| Boundary-integrity counters are observable | **verified in CI** | `integrity_last_24h` + dashboard tests | Metrics measure Human Queue events, not every provider-internal failure |
...
Human Queue uses this order when deciding whether to build:
...
When an upstream runtime fixes the original problem, Human Queue should record that as evidence against its own scope.

Example: OpenClaw users previously routed Slack exec approvals through another surface. Native Slack exec approvals later landed in [openclaw/openclaw#58155](https://github.com/openclaw/openclaw/pull/58155). That case no longer justifies a separate Human Queue approval connector.
...
- an operator with multiple runtimes/accounts uses Human Queue to resolve a real boundary;
- an operator says a native runtime fix is sufficient, removing a Human Queue use case;
