...
| Codex binary discovers and trusts human:// hooks | **verified with official Codex CLI 0.157.1** | real `codex app-server` `hooks/list` discovered the installed PermissionRequest hook, reported its current hash as `untrusted`, then reported `trusted` after Codex-native trust update | Still does not prove an authenticated model turn invokes the hook and resumes/denies the exact native tool call |
